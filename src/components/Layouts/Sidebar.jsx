...
function Sidebar({ collapsed, onToggle, currentPage, onPageChange }) {
    ...
    return (
        <div className={`${collapsed ? 'w-20' : 'w-72'
            ...
            {/* {Logo} */}
            <div div className='p-6 border-b border-slate-200/50 dark:border-slate-700/50'>
            ...
            <nav nav className='flex-1 p-4 space-y-2 overflow-y-auto'>
                {menuItems.map((item) => {
                    return (
                        <div key={item.id}>
                            <button className={`w-full flex items-center justify-between p-3 rounded-xl transition-all duration-200 ${currentPage === item.id || item.active ? 'bg-linear-to-r from-blue-500 to-purple-600 text-white shadow-lg shadow-blue-500/25' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/50'}`}
                                onClick={() => {
                                    if (item.submenu) {
            ...
            {!collapsed && (
                < div className='p-4 border-t border-slate-200/50 dark:border-slate-700/50'>
                    <div className='flex items-center space-x-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50'>
                        <img src="https://www.menswearr.com/cdn/shop/files/c97c09590448edb46836c170c0c2391c.webp" alt="user" className='w-10 h-10 rounded-full ring-2 ring-blue-500' />
